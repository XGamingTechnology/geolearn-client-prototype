...
type IconName = "home" | "case" | "question" | "assignment" | "result" | "class" | "data" | "media" | "gis" | "account" | "plus" | "more" | "layers";
...
const primaryNavigation: NavItem[] = [
  ...
  { href: "/teacher/cases", label: "Case", short: "Case", icon: "case" },
  { href: "/teacher/questions", label: "Bank Soal", short: "Soal", icon: "question" },
...
const titleByPath: Array<[string, string, string]> = [
  ["/teacher/cases", "Case", "Bangun konteks pembelajaran"],
  ["/teacher/questions", "Bank Soal", "Kelola pertanyaan dan instrumen"],
  ["/teacher/assignments", "Penugasan", "Atur kelas, jadwal, dan pengerjaan"],
  ["/teacher/results", "Hasil", "Tinjau jawaban dan pembelajaran"],
...
export function TeacherShell({ children, displayName, schoolName, canManageAccounts }: { children: ReactNode; displayName: string; schoolName: string | null; canManageAccounts: boolean }) {
  ...
  const visibleResources = resourceNavigation.filter((item) => !item.admin || canManageAccounts);
    ...
    <div className={styles.shell}>
      <aside className={styles.sidebar}>
        <Link className={styles.brand} href="/teacher">
          ...
          <span className={styles.brandText}><strong>GeoLearn</strong><small>Teacher Workspace</small></span>
        </Link>

        <div className={styles.navLabel}>Alur Utama</div>
        <nav className={styles.nav} aria-label="Alur utama guru">{renderNav(primaryNavigation)}</nav>

        <div className={styles.navLabel}>Sumber Daya</div>
        <nav className={styles.nav} aria-label="Sumber daya guru">{renderNav(visibleResources)}</nav>

        <div className={styles.sidebarFoot}>
          <div className={styles.sidebarTip}><strong>Spatial Thinking Workspace</strong><span>Bangun case, soal, penugasan, lalu tinjau hasil dalam satu alur.</span></div>
        </div>
      </aside>
      ...
      <div className={styles.workspace}>
        <header className={styles.topbar}>
          <div className={styles.context}>
            <span className={styles.contextIcon}><Icon name={primaryNavigation.find((item) => isActive(item.href))?.icon ?? visibleResources.find((item) => isActive(item.href))?.icon ?? "home"}/></span>
            <span className={styles.contextText}><strong>{context[1]}</strong><small>{context[2]}</small></span>
          </div>

          <div className={styles.actions}>
            <Link className={styles.quickButton} href="/teacher/cases"><Icon name="plus"/><span>Buat Case</span></Link>
      ...
      <nav className={styles.mobileNav} aria-label="Navigasi guru seluler">
        ...
        <Link className={["/teacher/results","/teacher/classes","/teacher/data","/teacher/media","/teacher/gis","/teacher/accounts","/teacher/more"].some((href) => pathname.startsWith(href)) ? styles.active : ""} href="/teacher/more"><Icon name="more"/><span>Lainnya</span></Link>
      </nav>
    </div>
  );
}
