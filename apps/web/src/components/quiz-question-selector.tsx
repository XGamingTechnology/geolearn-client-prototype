...
export function QuizQuestionSelector({questions}:{questions:QuizQuestionOption[]}){
  ...
  const buckets=useMemo(()=>{
    ...
    return Array.from(map.values()).map(bucket=>({...bucket,items:bucket.items.filter(item=>{
      const haystack=`${item.title} ${item.spatialMode} ${item.stimulusType??""} ${item.responseType??""}`.toLowerCase();
  ...
  return <div className="assessment-create-form">
    ...
    <label>Cari soal<input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Judul, spatial mode, stimulus…"/></label>
