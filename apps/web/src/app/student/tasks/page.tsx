        ...
        <div className="student-task-list">
          {tasks.map((task)=>{
            ...
            return <article className="student-task-row" key={task.id}>
              ...
              <div><span className="assignment-kicker">{task.quizTitle} · {task.itemCount} soal</span><h2>{task.title}</h2><p>{task.closesAt?<>Deadline <LocalDateTime value={task.closesAt}/></>:"Tanpa deadline"}</p></div>
