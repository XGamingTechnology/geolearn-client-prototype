...
async function assertScope(actor:TeacherSession,value:Scope){
  ...
  if(value==="SCHOOL"){const allowed=actor.role==="SYSTEM_ADMIN"||actor.role==="SCHOOL_ADMIN"||await hasStaffPermission(actor,"CONTENT_MANAGE_SCHOOL");if(!allowed)throw new AuthorizationError();}
...
export async function registerRemoteRasterDataset(input:{actor:TeacherSession;title:string;description:string;scope:string;tileUrl:string;bbox:Bbox;attribution?:string;sourceLabel?:string;sensor?:string;acquiredAt?:string;temporalLabel?:string;}){
  ...
  try{
    ...
    await client.query("commit");return datasetId;
